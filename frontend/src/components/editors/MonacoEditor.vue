...
import { connectMonacoTheme } from '../../core/monaco-theme'
...
onMounted(() => {
  ...
  disconnectTheme = connectMonacoTheme()
...
onUnmounted(() => { subscription?.dispose(); const model = editor?.getModel(); editor?.dispose(); model?.dispose(); disconnectTheme?.() })
</script>
