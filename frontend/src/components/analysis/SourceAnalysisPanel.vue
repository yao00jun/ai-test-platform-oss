  ...
  <section class="surface source-panel" role="region" aria-label="五类输入与源码证据">
    ...
    <div class="source-layout">
      ...
      <div v-if="selected" class="source-detail">
        ...
        <ul v-if="selected.diagnostics?.length" class="source-diagnostics" aria-label="解析诊断"><li v-for="(item, index) in selected.diagnostics" :key="index"><strong>{{ item.code }}</strong> · {{ item.path }}{{ item.line ? `:${item.line}` : '' }}<br>{{ item.message }}</li></ul>
        <template v-if="ready">
...
<style scoped>
.source-panel { padding: 22px; margin-block: 24px; min-width: 0; overflow-wrap: anywhere; }.source-heading { display: flex; align-items: start; justify-content: space-between; gap: 12px; flex-wrap: wrap; }.source-heading h2 { font-size: 18px; margin: 0; }h3 { margin: 0 0 12px; font-size: 15px; }h4 { margin: 18px 0 12px; font-size: 13px; }.input-dimensions { display: flex; flex-wrap: wrap; gap: 8px; margin: 18px 0; }.input-dimensions span { font-size: 11px; border: 1px solid var(--border); background: #f8f9fd; border-radius: 20px; padding: 6px 10px; }.source-form { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 15px; margin-block: 20px; }.source-form label { display: grid; gap: 7px; font-size: 12px; }.source-form input, .source-form textarea { box-sizing: border-box; width: 100%; min-width: 0; font: inherit; padding: 10px; color: var(--text); border: 1px solid var(--border); border-radius: 6px; background: white; }.source-form textarea { resize: vertical; }.source-form :disabled { background: #f6f7fa; }.source-wide { grid-column: 1 / -1; }.inline-actions { flex-wrap: wrap; }.source-success { color: #16815d; }.source-layout { display: grid; grid-template-columns: 205px minmax(0, 1fr); gap: 22px; border-top: 1px solid var(--border); padding-top: 22px; }.source-history button { display: grid; gap: 7px; text-align: left; width: 100%; font: inherit; font-size: 11px; color: var(--text); background: white; border: 1px solid var(--border); border-radius: 7px; padding: 12px; margin-bottom: 8px; cursor: pointer; }.source-history button.selected { border-color: var(--primary); background: #f2f6ff; }.source-history small { color: var(--muted); }.source-detail, .source-history { min-width: 0; }.source-origin { border-left: 2px solid #c9d8fa; padding-left: 10px; line-height: 1.6; }.source-diagnostics { padding: 14px 14px 14px 30px; background: #fff9ee; border: 1px solid #f0dcb7; border-radius: 6px; font-size: 12px; }.source-diagnostics li + li { margin-top: 10px; }.source-failure { color: #c74040; font-size: 12px; }.source-evidence-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }.source-evidence-grid article { min-width: 0; border: 1px solid var(--border); padding: 0 14px 8px; border-radius: 7px; font-size: 12px; max-height: 350px; overflow: auto; }.source-evidence-grid small { display: block; color: var(--muted); font-size: 11px; line-height: 1.7; margin-top: 5px; }.source-json { margin-block: 18px; font-size: 12px; }.source-json summary { cursor: pointer; }.source-json pre, .source-code pre { overflow: auto; white-space: pre; font-size: 12px; line-height: 1.7; background: #f6f8fc; border: 1px solid var(--border); border-radius: 6px; padding: 14px; max-height: 420px; }.source-files { display: flex; gap: 8px; flex-wrap: wrap; }.source-files button { min-width: 0; max-width: 100%; font: inherit; font-size: 11px; color: var(--primary); border: 1px solid var(--border); border-radius: 5px; padding: 7px 10px; background: white; cursor: pointer; overflow-wrap: anywhere; }.source-files span { font-size: 9px; color: var(--muted); margin-right: 7px; }.source-code { min-width: 0; margin-top: 20px; }.source-conflict { border-left: 3px solid #d78b27; padding: 12px; margin-bottom: 18px; font-size: 12px; }.source-empty { padding: 35px 12px; line-height: 1.7; }.source-empty p { max-width: 470px; font-size: 13px; }@media (max-width: 850px) { .source-layout { grid-template-columns: minmax(0, 1fr); }.source-history { max-height: 260px; overflow: auto; } }@media (max-width: 560px) { .source-panel { padding: 16px; }.source-form, .source-evidence-grid { grid-template-columns: minmax(0, 1fr); } }
</style>
