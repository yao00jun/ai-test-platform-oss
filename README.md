...
| 模型名称 | 例如 `gpt-4o`、`qwen-plus`，以公司提供的名字为准。填好服务地址和 API Key 后点右侧「获取」可以拉取服务商的模型列表下拉选择；服务商不提供列表接口时直接手填 |
| 单次调用超时（秒） | 每次模型请求的总时间上限，默认 600 秒，可填 5–3600。模型较慢时适当调大；它不是并发数 |
| 每分钟最多请求数 | 供应商有 RPM 限制时填写，例如 `5`；`0` 表示不额外限流。超出后任务会排队，不会丢请求 |
| 温度 | 生成随机性，通常保持默认值；范围 0–2 |
| 信任内部/自签名证书 | 只在服务地址是 `https://` 时出现。公司内网模型网关用的是自签名或公司自己签发的证书时，测试连接会报「TLS 握手失败」，勾上它即可；公网服务保持不勾 |
...
升级要在内网电脑上完成，旧文件夹里的数据和配置不需要搬家：

1. 在能上网的电脑下载新版本 zip，复制到内网电脑。
2. 在内网电脑把 zip 解压到一个**新的**文件夹，不要覆盖旧文件夹。
3. 打开新文件夹，双击 `升级.cmd`，把正在使用的旧文件夹拖进黑色窗口，然后按回车并确认。
4. 程序会先做一次完整备份，停止旧实例，创建“升级前”回退副本，替换程序文件，保留 `instance`、`data` 和 `.runtime`，再启动新版本并检查健康状态。
5. 看到“升级完成”后，用原来的网址打开平台。数据库迁移会在新版本启动时自动执行。

`升级.cmd` 只能从新版本文件夹运行。旧文件夹不能删除：它里面的绝对路径配置和回退副本仍然有用；如果 MySQL 数据目录在安装文件夹外，升级前副本不含数据库，但第 1 步的逻辑备份仍然包含数据库。

### 升级后回退

如果新版本启动失败，先双击新文件夹里的 `停止.cmd`。把升级后的旧文件夹改成另一个名字，再把同级的“升级前”副本改回原来的文件夹名，最后双击回退副本里的 `启动.cmd`。数据库需要回退时，用升级前的备份在一个新的空实例执行“恢复”。
