...
public class ModelSettingsService {
    ...
    private ModelSettings optional() {
        ...
        if (!rows.isEmpty()) {
            ...
            return new ModelSettings(row.get("base_url").toString(), secrets.decrypt(row.get("api_key").toString()), row.get("model_name").toString(), ((Number) row.get("temperature")).doubleValue(), ((Number) row.get("timeout_seconds")).intValue(), row.get("version").toString());
        }
    ...
    public Map<String, Object> save(Input input) {
        ...
        String key = input.apiKey() == null || input.apiKey().isBlank() || input.apiKey().equals(SecretProtector.MASK) ? (previous == null ? "" : previous.apiKey()) : input.apiKey();
        ...
        jdbc.update("INSERT INTO ai_model_config(id,base_url,api_key,model_name,temperature,timeout_seconds,updated_at) VALUES('company-default',?,?,?,?,?,?) ON DUPLICATE KEY UPDATE base_url=VALUES(base_url),api_key=VALUES(api_key),model_name=VALUES(model_name),temperature=VALUES(temperature),timeout_seconds=VALUES(timeout_seconds),version=version+1,updated_at=VALUES(updated_at)", baseUrl, secrets.encrypt(key), input.modelName().strip(), temperature, timeout, Timestamp.from(Instant.now()));
        return view();
    }
    public record Input(String baseUrl, String apiKey, String modelName, Double temperature, Integer timeoutSeconds) { }
}
