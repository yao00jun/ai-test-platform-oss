...
Windows 发行包提供 `升级.cmd`。在内网电脑上下载并解压新包到新目录，然后双击新包的 `升级.cmd`，把旧安装目录拖入窗口并确认。脚本会先调用旧目录的 `backup`，再停止旧实例，创建同级的“升级前”回退副本，只替换 `app.jar`、脚本、文档、迁移和发行元数据，保留 `instance`、`data`、`.runtime`；最后启动旧目录中的新文件并轮询 `/actuator/health`。不要再把四个目录手工复制到新包，配置中的绝对路径会让新包继续使用旧目录，手工复制会造成数据分叉。

升级失败时停止实例，把升级后的目录改名，把同级的“升级前”副本改回原名再启动。数据库数据目录位于安装目录外时，回退副本不包含数据库，应使用升级前的逻辑备份恢复到空实例。

Linux 发行包可执行 `scripts/aitest.sh upgrade --target /path/to/old-install --yes`，流程与 Windows 相同。
...
V28 为公司模型配置增加每分钟请求上限和可配置的单次调用总预算。`requests_per_minute=0` 表示不额外限流，允许值为 0–10000；`timeout_seconds` 允许 5–3600 秒，默认 600 秒。迁移只把仍为旧默认 120 秒的记录提升为 600 秒，其他已经保存的超时值、温度、证书信任和模型密钥保持不变。升级后应在模型设置页核对这些值，并确认 Flyway 只新增 V28 成功记录。
